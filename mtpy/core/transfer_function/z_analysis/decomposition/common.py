...
# ---------------------------------------------------------------------------
# GB-style initial guesses (single-site and joint)
# ---------------------------------------------------------------------------
#
# These helpers seed the GB cost function's optimisation. They live here
# because (a) they encode the GB parameter-vector layout
# (:func:`_unpack_x` / :func:`_unpack_x_joint`) which already lives in
# common, and (b) the joint-multistart orchestrator below consumes them
# via factory injection. The McNeice-Jones Phase 1 API
# (:func:`...mcneice_jones.decompose_mcneice_jones`) does the same, so
# centralising them here lets MJ avoid reaching into ``groom_bailey``.
# Every method that builds on the GB Pauli forward model
# (:func:`_estim_imp`) inherits these guesses; a future Bahr or
# Lilley-fitted method with a different parameter layout would supply
# its own.
...
# ---------------------------------------------------------------------------
# GB joint cost function and bounds
# ---------------------------------------------------------------------------
#
# These two helpers are the joint-multistart orchestrator's GB-specific
# math. They live here for the same reason as the canonical / rotated
# initial guesses above: every consumer (groom_bailey.py and
# mcneice_jones.py) injects them as factories into
# :func:`_solve_band_joint_multistart`, and centralising them lets MJ
# avoid reaching into ``groom_bailey``. They share the GB parameter
# layout already encoded in :func:`_unpack_x_joint` and
# :func:`_estim_imp` (the GB89 forward model), which also live here.
...
def _build_bounds_joint(
    ...
    Same per-parameter defaults as :func:`...groom_bailey._build_bounds`,
    replicated across all sites where applicable. ``bounds_override``
...
def _objfun_joint(
    ...
    single-site :func:`...groom_bailey._objfun` exactly, with the
    shared theta from ``x[0]`` and the per-site distortion / regional
    impedance pulled from the joint state vector via
    ...
    for-bit with single-site :func:`...groom_bailey._objfun` —
    verified by ``TestObjfunJointSingleSiteReduction``.
    """
