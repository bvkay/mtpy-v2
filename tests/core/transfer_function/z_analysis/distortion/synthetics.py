...
~~~~~~~~~~~~~~~~~~~~~~
The harness's ``C(strike, ...) = gain · R(strike) · T · S ·
R(strike).T`` form looks superficially different from the
:func:`...common._estim_imp` form
``Z_meas = R(-theta) · {gain · T · S · Z_strike} · R(theta)``, but
the two are algebraically equivalent: ``R(theta)`` in
:func:`_estim_imp` is the *clockwise-from-North* ("geographic-MT")
rotation matrix
``[[cos(theta), sin(theta)], [-sin(theta), cos(theta)]]``, which
satisfies ``R(theta).T = R(-theta)``. Substituting
``Z_meas = R_ccw(strike) · M · R_cw(strike)`` (the harness form)
with ``M = gain · T · S · Z_strike``, ``R_ccw = R(strike).T``,
``R_cw = R(strike)`` reproduces the :func:`_estim_imp` algebra
exactly. The integration tests in this directory verify that
synthetics generated by the harness are recovered (modulo the
known GB gain non-identifiability at moderate distortion levels)
by every implemented method. See the ``Convention`` paragraph in
:func:`_estim_imp` for the Lilley 1998 eq. 28 reference and the
geographic-azimuth interpretation of ``theta``.
